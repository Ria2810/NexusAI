...
def main():
    ...
    st.markdown(
        "<h1 style='text-align: center;'>📊🤖 AI Data Visualization Agent 🤖📊</h1>", 
        unsafe_allow_html=True
    )
    st.write("Upload your dataset and ask questions about it!")
    
    # Load API keys from .env and set in session_state
    together_api_key = os.getenv("TOGETHER_API_KEY")
    e2b_api_key = os.getenv("E2B_API_KEY")
    if not together_api_key or not e2b_api_key:
        st.error("API keys not found in the .env file. Please ensure TOGETHER_API_KEY and E2B_API_KEY are set.")
        return
    st.session_state.together_api_key = together_api_key
    st.session_state.e2b_api_key = e2b_api_key
    
    # Initialize session state variable for model if not already set
    ...
    if uploaded_file is not None:
        ...
        if st.button("Analyze"):
            with Sandbox(api_key=st.session_state.e2b_api_key) as code_interpreter:
                ...
                # Get LLM response and execute code if provided
                code_results, llm_response = chat_with_llm(code_interpreter, query, dataset_path)
                
                # Display the User Query in a styled box with emoji
                st.markdown(
                    f"<div style='background-color: #3A506B; padding: 10px; border-radius: 5px; margin-bottom: 10px; color: white;'>"
                    f"<strong>👤 User Query:</strong> {query}</div>",
                    unsafe_allow_html=True
                )
                
                # Display the AI Response in a styled box with emoji
                st.markdown(
                    f"<div style='background-color: #3A506B; padding: 10px; border-radius: 5px; margin-bottom: 10px; color: white;'>"
                    f"<strong>🤖 AI Response:</strong> {llm_response}</div>",
                    unsafe_allow_html=True
                )
