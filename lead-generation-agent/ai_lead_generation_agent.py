...
st.set_page_config(page_title="AI Lead Generation Agent🎯", page_icon=":dart:")
...
def main():
    ...
    # Enhanced Sidebar with description, features, and example queries, along with input for number of links
    with st.sidebar:
        st.markdown("# AI Lead Generation Agent")
        st.markdown("## Overview")
        st.write(
            "This agent uses advanced search capabilities combined with AI to generate leads from Quora posts. "
            "It transforms your detailed query into a concise description, finds relevant Quora URLs, extracts user data such as username, bio, post type, and upvotes, and automatically writes this information into a new Google Sheet."
        )
        st.markdown("## Features")
        st.write("- **Query Transformation:** Converts detailed user queries into focused company descriptions.")
        st.write("- **URL Search:** Uses the Firecrawl API to search for relevant Quora URLs.")
        st.write("- **User Data Extraction:** Extracts comprehensive user information from Quora posts.")
        st.write("- **Google Sheets Integration:** Writes the extracted data into a new Google Sheet for easy analysis.")
        st.markdown("## Example Queries")
        st.write("- Looking for users interested in AI-powered customer support solutions")
        st.write("- Find leads for automated video editing software")
        st.write("- Generate leads for ML fraud detection tools")
        st.write("- Search for users asking about voice cloning technology")
        num_links = st.number_input("Number of links to search", min_value=1, max_value=10, value=3)
        if st.button("Reset"):
            st.session_state.clear()
            st.experimental_rerun()
    
    # Ensure the API keys are available from the .env file
    if not all([FIRECRAWL_API_KEY, OPENAI_API_KEY, COMPOSIO_API_KEY]):
        st.error("Missing API keys. Please ensure your .env file contains FIRECRAWL_API_KEY, OPENAI_API_KEY, and COMPOSIO_API_KEY.")
        return
